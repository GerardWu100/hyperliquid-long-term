...
developer documentation. Product code lives under `src/hl_candles/`. Thin
operator scripts live under `scripts/`. The detailed project contract is
`docs/reference/IMPLEMENTATION_PLAN.md`.

```text
Hyperliquid REST -> hl_candles service -> ClickHouse tables/views
                         |
...
- `src/hl_candles/`: importable service package.
...
Start with `README.md`, then `src/hl_candles/app.py` for orchestration.
